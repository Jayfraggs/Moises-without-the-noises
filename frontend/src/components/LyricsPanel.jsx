...
/**
 * Displays timestamped lyrics synchronized to AudioEngine playback.
 *
 * Segments are precomputed word-level boundaries from Whisper, arranged as
 * paragraph-level blocks. Within each segment, individual words are rendered
 * and highlighted as playback position reaches their time windows.
 *
 * Uses requestAnimationFrame (same pattern as NoteDisplay) to poll
 * engine.getCurrentTime() and track the active segment and word, auto-scrolling
 * to keep active content visible.
 */
export function LyricsPanel({ engine, lyrics, isPlaying }) {
  ...
  // Reset cursor when lyrics change (different song or data refreshed).
  useEffect(() => {
    cursorRef.current = 0;
  }, [lyrics]);

  // Auto-scroll to active segment when it changes (not on every word change).
  useEffect(() => {
    if (activeSegmentRef.current && activeSegmentIdx !== null) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      // Detect backward playback jump (seek, loop, scrub) -- reset scan.
      ...
      while (idx < segments.length - 1 && segments[idx].end <= t) {
        idx++;
      }
      cursorRef.current = idx;

      const seg = segments[idx];
      const isSegmentActive = seg && t >= seg.start && t < seg.end;

      if (isSegmentActive) {
        setActiveSegmentIdx(idx);

        // Find active word within the active segment.
        if (seg.words && seg.words.length > 0) {
          let wordIdx = 0;
          while (
            wordIdx < seg.words.length - 1 &&
            seg.words[wordIdx].end <= t
          ) {
            wordIdx++;
          }

          const word = seg.words[wordIdx];
          const isWordActive = word && t >= word.start && t < word.end;
          setActiveWordIdx(isWordActive ? wordIdx : null);
        } else {
  ...
  }, [isPlaying, lyrics, engine]);

  const hasLyrics = lyrics && lyrics.available && lyrics.segments;

  if (!hasLyrics) {
    return (
      <div className="lyrics-panel">
        <div className="lyrics-panel__empty">No lyrics available</div>
      </div>
    );
  }
  ...
  return (
    <div className="lyrics-panel" ref={containerRef}>
      {segments.map((segment, segIdx) => (
        <p
          key={segIdx}
          ref={segIdx === activeSegmentIdx ? activeSegmentRef : null}
          className={`lyrics-panel__segment ${
            segIdx === activeSegmentIdx ? 'is-active-segment' : ''
          }`}
        >
          {segment.words && segment.words.length > 0 ? (
            segment.words.map((word, wordIdx) => (
              <span
                ...
                className={`lyrics-panel__word ${
                  segIdx === activeSegmentIdx && wordIdx === activeWordIdx
                    ? 'is-active-word'
                    : ''
                }`}
              >
            ...
            <span className="lyrics-panel__word">{segment.text}</span>
          )}
        </p>
      ))}
    </div>
